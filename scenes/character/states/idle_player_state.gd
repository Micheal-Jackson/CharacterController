...
func update(_delta : float) -> void:
	...
	if state_machine.character.velocity.length() > 0.0 and Input.is_action_pressed("sprint") and state_machine.character.is_on_floor():
		transition.emit("SprintingPlayerState")
		
	elif state_machine.character.velocity.length() > 0.0 and !Input.is_action_pressed("sprint") and state_machine.character.is_on_floor():
